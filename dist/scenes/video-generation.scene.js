...
const akool_file_uploader_1 = require("../utils/akool-file-uploader");
        ...
        try {
            ...
            try {
                ...
                if (file.file_path) {
                    ...
                    if (!akool_file_uploader_1.AkoolFileUploader.isSupportedImageFormat(fileName, 'image/jpeg')) {
                        await ctx.reply("❌ Неподдерживаемый формат файла для AKOOL!\n\n" +
                    ...
                    if (file.file_size && file.file_size > 10 * 1024 * 1024) {
                        await ctx.reply("❌ Файл слишком большой для AKOOL!\n\n" +
        ...
        try {
            ...
            try {
                ...
                if (file.file_path) {
                    ...
                    if (file.file_size && file.file_size > 10 * 1024 * 1024) {
                        await ctx.reply("❌ Голосовое сообщение слишком большое!\n\n" +
                            `Размер: ${Math.round(file.file_size / (1024 * 1024) * 100) / 100} МБ\n` +
                            "Максимум: 10 МБ\n\n" +
                    ...
                    if (fileName.includes('.oga') || fileName.includes('.ogg')) {
                        await ctx.reply("⚠️ Обнаружен формат OGA/OGG\n\n" +
