...
export class DidService {
  ...
  async generateVideo(request: VideoGenerationRequest): Promise<VideoGenerationResponse> {
    ...
    try {
      ...
      this.logger.debug(
        `[${requestId}] Payload config: quality=${payload.config.quality}, resolution=${payload.config.output_resolution}`
      );
      this.logger.log(
        `[${requestId}] 🎵 Script type: ${payload.script.type}${payload.script.provider ? ` (TTS: ${payload.script.provider.voice_id})` : ' (Custom Audio)'}`
      );
      ...
      this.logger.debug(
        `[${requestId}] 📥 D-ID API response: ${response.status} ${response.statusText}`
      );
      ...
      this.logger.log(
        `[${requestId}] ✅ Video generation started successfully with ID: ${result.id}`
      );
  ...
  async getVideoStatus(videoId: string): Promise<VideoGenerationResponse> {
    try {
      ...
      if (result.status === 'done' && result.result_url) {
        this.logger.log(
          `✅ Video ${videoId} completed successfully with URL: ${result.result_url}`
        );
  ...
  async uploadAudio(audioBuffer: Buffer): Promise<string> {
    ...
    try {
      ...
      this.logger.debug(
        `[${uploadId}] 📥 Audio upload response: ${response.status} ${response.statusText}`
      );
  ...
  async uploadImage(imageBuffer: Buffer): Promise<string> {
    ...
    try {
      ...
      this.logger.debug(
        `[${uploadId}] 📥 Image upload response: ${response.status} ${response.statusText}`
      );
